      ...
      <header className="tw-bg-forest tw-text-white tw-p-6">
        <div className="tw-container tw-mx-auto tw-flex tw-justify-between tw-items-center">
          ...
          <div className="tw-flex tw-items-center">
            <span className="tw-text-white tw-mr-4">{user?.firstName || user?.emailAddresses[0]?.emailAddress}</span>
      ...
      <main className="tw-flex-grow tw-container tw-mx-auto tw-p-6">
        <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-6 tw-mb-8">
          <div className="tw-bg-white tw-rounded-lg tw-shadow-md tw-p-6 tw-border-l-4 tw-border-blue-500">
            ...
            <p className="tw-text-3xl tw-font-bold">{searches.filter(s => s.isActive).length}</p>
          </div>
        ...
        <div className="tw-bg-white tw-rounded-lg tw-shadow-md tw-mb-8">
          <div className="tw-p-6 tw-border-b tw-border-gray-200 tw-flex tw-justify-between tw-items-center">
            <h2 className="tw-text-2xl tw-font-semibold">Your Property Searches</h2>
            <Link 
              href="/searches/new" 
          ...
          <div className="tw-p-6">
            ...
            {searches.length === 0 ? (
              <div className="tw-text-center tw-py-8">
                <p className="tw-text-gray-500 tw-mb-4">You don't have any property searches yet.</p>
                <Link 
                  href="/searches/new" 
                ...
                <table className="tw-min-w-full tw-border-collapse">
                  <thead>
                    <tr className="tw-bg-gray-50">
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Name</th>
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Location</th>
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Price Range</th>
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Beds/Baths</th>
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Last Checked</th>
                      <th className="tw-py-3 tw-px-4 tw-text-left tw-font-semibold tw-text-gray-600">Status</th>
                      <th className="tw-py-3 tw-px-4 tw-text-right tw-font-semibold tw-text-gray-600">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {searches.map((search) => (
                      <tr key={search.id} className="tw-border-t tw-border-gray-200">
                        ...
                        <td className="tw-py-3 tw-px-4">
                          <span className={`tw-inline-block tw-px-2 tw-py-1 tw-rounded-full tw-text-xs tw-font-semibold ${
                            search.isActive 
        ...
        <div className="tw-bg-white tw-rounded-lg tw-shadow-md">
          <div className="tw-p-6 tw-border-b tw-border-gray-200">
            <h2 className="tw-text-2xl tw-font-semibold">Recently Found Properties</h2>
          </div>
          <div className="tw-p-6">
            {properties.length === 0 ? (
              <div className="tw-text-center tw-py-8">
                <p className="tw-text-gray-500">No properties found yet.</p>
                <p className="tw-text-gray-500 tw-mt-2">Create a search to start finding properties in the DMV area.</p>
              </div>
            ) : (
              <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 lg:tw-grid-cols-3 tw-gap-6">
                {properties.map((property) => (
                  <div key={property.id} className="tw-border tw-border-gray-200 tw-rounded-lg tw-overflow-hidden">
                    <div className="tw-relative tw-pb-[60%] tw-bg-gray-100">
                      ...
                      <div className="tw-absolute tw-bottom-0 tw-left-0 tw-right-0 tw-bg-gradient-to-t tw-from-black/70 tw-to-transparent tw-p-4">
                        <p className="tw-text-white tw-font-bold tw-text-xl">${property.price.toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="tw-p-4">
                      <h3 className="tw-font-semibold tw-mb-1 tw-line-clamp-1">{property.address}</h3>
                      <p className="tw-text-gray-600 tw-text-sm tw-mb-2">{property.city}, {property.state}</p>
                      <p className="tw-mb-3">
                        <span className="tw-font-medium">{property.bedrooms}</span> bd | 
                        <span className="tw-font-medium tw-ml-1">{property.bathrooms}</span> ba
                        {property.squareFeet && (
                          <> | <span className="tw-font-medium">{property.squareFeet.toLocaleString()}</span> sqft</>
                        )}
                      </p>
                      <a 
                        href={property.url} 
